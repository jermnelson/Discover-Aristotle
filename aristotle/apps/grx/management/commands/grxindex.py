...
class Command(mb.BaseCommand):
    ...
    def handle(self, *file_or_urls, **options):
        ...
        if new:
            ...
            r = urllib2.Request(conf.settings.GRX_SOLR_URL + 'update?commit=true')
...
def get_multi():
    ...
    schema = et.parse(conf.settings.GRX_SOLR_DIR + 'conf/schema.xml')
...
def load_solr(csv_file):
    ...
    update_url = conf.settings.GRX_SOLR_URL + 'update/csv?%s'
