...
import catalog.settings as catalog_settings
...
def display_ill(record):
  ...
  is finished</li>''' % record.get('title_display'))
    return ''
...
def display_online(record):
    ...
    """
    if record.has_key('url_fulltext_display'):
        return mark_safe('''<li><a href="%s">Access</a> <em>%s</em> online</li>''' % (record.get('url_fulltext_display')[0],
...
def generate_prospector_url(record_id):
    ...
    """
    prospector_url = catalog_settings.PROSPECTOR_URL % (record_id,record_id)
...
def generate_search_filter(param):
    ...
    if param[0] == 'q':
        raw_query = param[1]
        logging.error("RAW QUERY is %s" % raw_query)
        if type(raw_query) == list:
            term_list = [(v,'q') for v in raw_query]
        elif raw_query.count('AND') > 0: # For AND boolean operator
...
register.filter("display_ill",
                display_ill)
register.filter("display_online",
                display_online)
...
register.filter('generate_prospector_url',
                generate_prospector_url)
