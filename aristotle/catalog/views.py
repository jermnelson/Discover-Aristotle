...
def rpc_router(request):
    """
    Routes rpc requests to correct method.
    """
    if request.method == 'POST':
        if not request.POST.has_key('func'):
            raise Http404
        func = request.POST['func']
        func_args = request.POST
    else:
        if not request.GET.has_key('func'):
            raise Http404
        func = request.GET['func']
        func_args = request.GET
    # RPC switch to functions
    if func == 'search':
        output = rpc_search(func_args)
    return HttpResponse(output)

def rpc_search(func_args):
    """Method accepts either a GET or POST and returns a rendered
    results template
    """
    if not func_args.has_key('search-type'):
        search_type = 'keyword'
    else:
        search_type = func_args['search-type']
    result_template = get_template('results-list.html')
    if search_type == 'keyword' or search_type == 'search':
        start = int(func_args.get('page_index')) * int(func_args.get('rows'))
        query_result = solr_server.search(q=func_args.getlist('q'),
                                          fl="*",
                                          fq=func_args.getlist('fq'),
                                          facet=True,
        ...
        query_result = None
    return result_template.render(Context({'catalog_results':query_result}))
